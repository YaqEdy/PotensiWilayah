...
-- ----------------------------
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_pend_non_formal_temp` AS select `a`.`id_pend_non_formal` AS `id_pend_non_formal`,`a`.`id_ktp` AS `id_ktp`,`a`.`nama_pend` AS `nama_pend`,`a`.`jenis_pend` AS `jenis_pend`,`a`.`tahun` AS `tahun`,`a`.`ket` AS `ket`,`a`.`instansi` AS `instansi`,`a`.`idsession` AS `idsession`,`b`.`nama_ktp` AS `nama_ktp`,`c`.`nama_instansi` AS `nama_instansi`,`b`.`jekel` AS `jekel`,`b`.`tanggal_lahir` AS `tanggal_lahir`,`d`.`id_jns_bantuan` AS `id_jns_bantuan`,`d`.`jns_bantuan` AS `jns_bantuan` from (((`tbl_m_pend_non_formal_temp` `a` join `master_ktp` `b` on((`a`.`id_ktp` = `b`.`id_ktp`))) join `tbl_m_instansi` `c` on((`a`.`instansi` = `c`.`id_instansi`))) left join `vw_t_bantuan` `d` on((`a`.`id_ktp` = `d`.`id_ktp`)));
...
-- ----------------------------
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_t_bantuan_temp` AS select `a`.`id_t_bantuan` AS `id_t_bantuan`,`a`.`id_jns_bantuan` AS `id_jns_bantuan`,`d`.`jns_bantuan` AS `jns_bantuan`,`a`.`id_m_instansi` AS `id_m_instansi`,`b`.`nama_instansi` AS `nama_instansi`,`a`.`id_ktp` AS `id_ktp`,`a`.`nama_bantuan` AS `nama_bantuan`,`a`.`ket` AS `ket`,`a`.`tgl_bantuan` AS `tgl_bantuan`,`a`.`idsession` AS `idsession`,`c`.`nama_ktp` AS `nama_ktp`,`c`.`tempat_lahir` AS `tempat_lahir`,`c`.`tanggal_lahir` AS `tanggal_lahir`,`c`.`jekel` AS `jekel`,`c`.`gol_darah` AS `gol_darah`,`c`.`alamat` AS `alamat`,`c`.`rt` AS `rt`,`c`.`rw` AS `rw`,`c`.`id_kel` AS `id_kel`,`c`.`id_kec` AS `id_kec`,`c`.`agama` AS `agama`,`c`.`status_kawin` AS `status_kawin`,`c`.`pekerjaan` AS `pekerjaan`,`c`.`warga_negara` AS `warga_negara`,`c`.`link_gambar` AS `link_gambar`,`c`.`is_delete` AS `is_delete` from (((`tbl_t_bantuan_temp` `a` join `tbl_m_instansi` `b` on((`a`.`id_m_instansi` = `b`.`id_instansi`))) join `master_ktp` `c` on((`a`.`id_ktp` = `c`.`id_ktp`))) left join `tbl_r_jns_bantuan` `d` on((`a`.`id_jns_bantuan` = `d`.`id_jns_bantuan`)));
...
INSERT INTO `tbl_m_pend_non_formal_temp` VALUES ('25', '9887', 'mesin', 'training', '2017', 'opoooo', '000002', '5e15ab65-b531-11e8-b64e-782bcbdbdcb7');
...
INSERT INTO `tbl_t_anggota_komunitas_temp` VALUES ('94', '9887', 'a5149c7e-b523-11e8-b5ce-782bcbdbdcb7');
INSERT INTO `tbl_t_bantuan` VALUES ('56', '28347c80-aec7-11e8-9980-782bcbdbdcb7', '1', '000001', '8998789', '2018-09-02', 'sembako', 'makan', '40', '2018-09-02 22:45:08', null, null);
