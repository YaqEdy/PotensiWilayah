...
class Master_pend_non_formal_2 extends CI_Controller {
    ...
    function home() {
        ...
        if (isset($_POST["btnSimpan"])) {
            ...
            $this->template->load('template/template_dataTable', 'master/master_pend_non_formal_2_v', $data);
        }
    }
    ...
    public function getBantuanAll() {
        ...
        $rows=$this->global_m->get_data("SELECT * FROM vw_pend_non_formal where id_ktp=$iPID");
    ...
    public function getBantuan() {
        ...
        // print_r($iPID);die();
        $this->CI = & get_instance(); //and a.kcab_id<>'1100'
        $rows=$this->global_m->get_data("SELECT * FROM vw_t_bantuan where id_ktp=".$iPID);
        $data['data'] = array();
        $no=1;
        foreach ($rows as $row) {
            $array = array(
    ...
    function ajax_save() {
        ...
        $data = array(
            ...
            $model = $this->global_m->simpan('tbl_m_pend_non_formal',$data);            
            $itipePesan='success';
            $ipesan='Pendidikan non formal berhasil disimpan.';
        ...
        if ($model) {
            ...
            $array = array(
                'act' => false,
                'tipePesan' => 'error',
                'pesan' => 'Pendidikan non formal gagal disimpan.'
            );
        }
        $this->output->set_output(json_encode($array));
    }
