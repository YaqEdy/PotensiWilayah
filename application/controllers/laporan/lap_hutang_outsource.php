...
class Lap_hutang_outsource extends CI_Controller {

    function __construct() {
        parent::__construct();

        $this->load->model('home_m');
        $this->load->model('laporan/lap_hutang_outsource_m');
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('laporan/lap_hutang_outsource/home');
        ...
        if (isset($_POST["btnSimpan"])) {
            ...
            $this->template->load('template/template_dataTable', 'laporan/lap_hutang_outsource_v', $data);
        }
    }
    ...
    public function getCucianAmbil() {
        ...
        $idOutsource = $this->input->post('idOutsource', TRUE);
        
        $rows = $this->lap_hutang_outsource_m->getCucianAmbil($tglAwal, $tglAkhir,$idOutsource);
        $data['data'] = array();
        $i = 0;
        $noSeq = 1;
        $ttl_harga = 0;
