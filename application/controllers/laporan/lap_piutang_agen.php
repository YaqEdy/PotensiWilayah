...
class Lap_piutang_agen extends CI_Controller {

    function __construct() {
        parent::__construct();

        $this->load->model('home_m');
        $this->load->model('laporan/lap_piutang_agen_m');
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('laporan/lap_piutang_agen/home');
        ...
        if (isset($_POST["btnSimpan"])) {
            ...
            $this->template->load('template/template_dataTable', 'laporan/lap_piutang_agen_v', $data);
        }
    }
    ...
    public function getCucianAmbil() {
        ...
        $rows = $this->lap_piutang_agen_m->getCucianAmbil($tglAwal, $tglAkhir,$idAgen);
        $data['data'] = array();
        $i = 0;
        $noSeq = 1;
        $ttl_harga = 0;
