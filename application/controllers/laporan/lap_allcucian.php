...
class Lap_allcucian extends CI_Controller {

    function __construct() {
        parent::__construct();

        $this->load->model('home_m');
        $this->load->model('laporan/lap_allcucian_m');
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('laporan/lap_allcucian/home');
        ...
        if (isset($_POST["btnSimpan"])) {
            ...
            $this->template->load('template/template_dataTable', 'laporan/lap_allcucian_v', $data);
        }
    }
    ...
    public function getCucianAmbil() {
        ...
        $rows = $this->lap_allcucian_m->getCucianAmbil($tglAwal, $tglAkhir);
        $data['data'] = array();
        $i = 0;
        $noSeq = 1;
        foreach ($rows as $row) {
