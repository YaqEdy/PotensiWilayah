...
class Trans_po extends CI_Controller {
    ...
    function cetakPO($id_master) {
        if ($this->auth->is_logged_in() == false) {
            ...
            $data['isidatamaster'] = $this->trans_po_m->cetakPOMaster($id_master);
            //print_r($data['isidatamaster']);
            $data['isidatatrans'] = $this->trans_po_m->cetakPOTrans($id_master);
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('transaksi/trans_po/home');
        ...
        $data['produk'] = $this->trans_po_m->getProduk();
        $data['cust'] = $this->trans_po_m->getCustomer();
        $data['agen'] = $this->trans_po_m->getAgen();
        $data['karyawan'] = $this->sec_user_m->getAllKaryawan();
        $data['osource'] = $this->trans_po_m->getAllOutsource();
        ...
        if (isset($_POST["btnSimpan"])) {
            ...
            $this->template->load('template/template_dataTable', 'transaksi/trans_po_v', $data);
        }
    }
    ...
    function getDescProduk() {
        ...
        $rows = $this->trans_po_m->getDescProduk($idProduk);
        if ($rows) {
            foreach ($rows as $row)
                $array = array(
                    'baris' => 1,
    ...
    function simpan() {
        ...
        $idMasterCm = $this->trans_po_m->getIdMasterIn($bulan, $tahun);
        //$noPO = $this->trans_po_m->getNoPO($bulan, $tahun);
        ...
        $model = $this->trans_po_m->insertMaster($dataMaster);
        ...
        if ($totJurnal > 0) {
            for ($i = 1; $i <= $totJurnal; $i++) {
                ...
                if ($tmpKdProduk <> '') {
                    ...
                    $query = $this->trans_po_m->insertTransInOut($data);
                    ...
                    //$queryUpdateMProduk = $this->trans_po_m->updateMProduk($tmpKg, $tmpKdProduk);
                }
            }
        }
        $dataMaster = array(
        ...
        $model = $this->trans_po_m->ubahMaster($dataMaster, $idMasterCm);
        ...
        if ($idAgen <> '') {
            ...
            $getInfoAgen = $this->trans_po_m->getInfoAgen($idAgen);
        ...
        if ($status_bayar == 1) {
            ...
            $model = $this->trans_po_m->ubahMaster($dataMaster_sdhbayar, $idMasterCm);
    ...
    public function getCucianMasuk() {
        ...
        $rows = $this->trans_po_m->getCucianMasuk($tglAwal, $tglAkhir);
        $data['data'] = array();
        $i = 0;
        $noSeq = 1;
        foreach ($rows as $row) {
    ...
    function getCMDetail() {
        ...
        $crows = $this->trans_po_m->getJmlRincianCM($idMaster);
        if ($crows <= 0) {
            $array = array('baris' => 0);
            $rows['data_cpa'] = $array;
            $this->output->set_output(json_encode($rows));
        } else {
            $rows = $this->trans_po_m->getRincianCM($idMaster);
            $this->output->set_output(json_encode($rows));
        }
    }
    ...
    function simpanAmbil() {
        ...
        $statusSelesai = $this->trans_po_m->getStatusSelesai($idMaster);
        $statusOutsource = $this->trans_po_m->getStatusOutsource($idMaster);
        ...
        if ($statusSelesai == 0 || $statusOutsource == 1) {
            ...
            $getStatusBayar = $this->trans_po_m->getStatusBayar($idMaster);
            ...
            if ($getStatusBayar == 1) {
                ...
                $model = $this->trans_po_m->ubahMaster($dataMaster, $idMaster);
            } else {
                $dataMaster = array(
                    'tgl_ambil' => $tglTrans,
                    'waktu_ambil' => date("H:i:s"),
                ...
                if ($stsBayar == 1 && $jmlBayar > 0) {
                    $piutang = $this->trans_po_m->getPiutang($idMaster);
    ...
    function simpanFinish() {
        ...
        if ($statusOutsource == 1) {
            ...
            if ($idKaryawan <> '') {
                ...
                $getInfoKyw = $this->trans_po_m->getInfoKyw($idKaryawan);
    ...
    function simpanOutsource() {
        ...
        $getInfoOutsource = $this->trans_po_m->getInfoOutsource($idOutsource);
        $getIntOutsource = $this->akuntansi_m->getkodeperk('biaya_outsource');

        $hargaCucian = 0;
        foreach ($data_idTransOS as $row_idtransos) {
            if (trim($row_idtransos) != '') {
                $rows = $this->trans_po_m->getInfoTransCM(trim($row_idtransos));
                $hargaCucian = $hargaCucian + $rows[0]->harga_cucian;
                $dataTrans = array(
                    'status_outsource' => 1
                );
                $rows = $this->trans_po_m->ubahTrans($dataTrans, trim($row_idtransos));
            }
        }
        //$rows = $this->trans_po_m->getRincianCM($idMaster);
    ...
    function simpanOutsourceMsk() {//kembali ke laundry
        ...
        $jml_detail_cucian = $this->trans_po_m->getJmlRincianCM($idMaster);
        $jml_detail_cucian_os = $this->trans_po_m->getJmlRincianOS($idMaster);
...
/* $act ="<div class='btn-group'>
              <button type='button' class='btn btn-sm btn-default'>Detail</button>
              ...
              <li class='divider'> </li>
              <li>
              <a href='javascript:;'> Cetak </a>
              </li>
              </ul>
              </div>"; 

            $act = "<div class='btn-group'>
                    <a aria-expanded='false' class='btn btn-default dropdown-toggle' data-toggle='dropdown' href='javascript:;'> Act
                        <i class='fa fa-angle-down'></i>
                    </a>
                    <ul class='dropdown-menu'>
                        <li>
                            <a class='btn btn-primary' href='javascript:;'> Ubah
                            </a>
                        </li>
                        <li>
                            <a class='btn btn-danger cls_btnAmbil' href='javascript:;' data-target='#idDivAmbilProduk'
                                           id='id_btnModalAmbil' data-toggle='modal' onclick='ambil(tr_cm" . $noSeq . ")'> Ambil </a>
                        </li>
                        <li>
                            <a class='btn btn-warning' href='javascript:;'> Keluar </a>
                        </li>
                        <li>
                            <a class='btn btn-success' href='javascript:;'> Cetak
                            </a>
                        </li>
                    </ul>
                </div>";
