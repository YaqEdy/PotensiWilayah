...
class Trans_masuk extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('transaksi/trans_masuk_m');
        session_start();
    }
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('transaksi/trans_masuk/home');
        ...
        $data['produk'] = $this->trans_masuk_m->getProduk();
        $data['storage'] = $this->trans_masuk_m->getStorage();
        $data['suplier'] = $this->trans_masuk_m->getSuplier();
        ...
        if (isset($_POST["btnSimpan"])) {
            ...
            $this->template->load('template/template_dataTable', 'transaksi/trans_masuk_v', $data);
        }
    }
    ...
    function getDescProduk() {
        ...
        $rows = $this->trans_masuk_m->getDescProduk($idProduk);
        if ($rows) {
            foreach ($rows as $row)
                $array = array(
                    'baris' => 1,
    ...
    function getDescStorage() {
        ...
        $rows = $this->trans_masuk_m->getDescStorage($idStorage);
        if ($rows) {
            foreach ($rows as $row)
                $array = array(
                    'baris' => 1,
    ...
    function simpan() {
        ...
        $selisihLt = str_replace(',', '', trim($this->input->post('selisihLt')));
        ...
        $idMasterIn = $this->trans_masuk_m->getIdMasterIn($bulan, $tahun);
        ...
        $data = array(
            ...
            'selisih_lt' => $selisihLt,
            'selisih_persen' => $selisihPersen,
            'keterangan' => $ket,
            'userid' => $this->session->userdata('id_user')
        );
        $model = $this->trans_masuk_m->insertMaster($data);
        ...
        if ($totJurnal > 0) {
            for ($i = 1; $i <= $totJurnal; $i++) {
                ...
                if ($tmpKdProduk <> '') {
                    ...
                    $query = $this->trans_masuk_m->insertTransInOut($data);
                    $queryUpdateMProduk = $this->trans_masuk_m->updateMProduk($tmpKg, $tmpKdProduk);
                }
            }
        }
...
//        $rows = $this->trans_masuk_m->getKywAll();
//        $data['data'] = array();
//        foreach ($rows as $row) {
//            $array = array(
...
//    {
//        $this->CI =& get_instance();//and a.kcab_id<>'1100'
//        $rows = $this->trans_masuk_m->getAdvAll();
//        $data['data'] = array();
//        foreach ($rows as $row) {
...
//        $rows = $this->trans_masuk_m->getAdvReq($requester);
//        $data['data'] = array();
//        foreach ($rows as $row) {
//            $jmlUang = number_format($row->jml_uang, 2);
//            $array = array(
...
//        $rows = $this->trans_masuk_m->getDescKurs($idKurs);
//        if ($rows) {
//            foreach ($rows as $row)
...
//        $crows = $this->trans_masuk_m->getCDescCpa($idAdv);
//        if ($crows <= 0) {
//            $array = array('baris' => 0);
//            $rows['data_cpa'] = $array;
//            $this->output->set_output(json_encode($rows));
//        } else {
//            $rows = $this->trans_masuk_m->getDescCpa($idAdv);
//            $this->output->set_output(json_encode($rows));
//        }
//    }
//
...
//            //        		''		        	=>$,
//        );
//        $model = $this->trans_masuk_m->updateAdv($data, $idAdv);
//		$model = $this->trans_masuk_m->deleteCpa($idAdv);
//        $totJurnal = trim($this->input->post('txtTempLoop'));
//        if($totJurnal > 0){
//            $query=$this->trans_masuk_m->deleteCpa($idAdv);
//
//            for($i=1;$i<=$totJurnal;$i++){
...
//                    'jumlah'        => $tmpJumlah
//                );
//				$budget = $this->trans_masuk_m->get_saldo_cflow($tmpKodeCflow);
//				if ($budget < $tmpJumlah){
//					$dataBudget = array(
//						'inout_budget'         => 1
//					);
//					$queryUpdate = $this->trans_masuk_m->updateAdv($dataBudget,$idAdv);	
//				}
//                $query=$this->trans_masuk_m->insertCpa($data);
//            }
//        }else{
//            $query=$this->trans_masuk_m->deleteCpa($idAdv);
//        }
...
//        $model = $this->trans_masuk_m->deleteAdv($idAdvance);
//
//
//        if ($totJurnal > 0) {
//            $query = $this->trans_masuk_m->deleteCpa($idAdvance);
//        }
//
//        if ($model) {
//            $array = array(
...
//            'app_user_id' => $flag
//        );
//        $model 		= $this->trans_masuk_m->updateAdv($data,$idAdvance);
//
//        if($model){
//            $array = array(
//                'act'	=>1,
...
//            $data['advance'] = $this->trans_masuk_m->getDescAdv($idAdv);
...
//        }else{
//            $data['info'] = $this->setting_laporan_m->getAllSetting();
//            $data['advance'] = $this->trans_masuk_m->cetak_cpa($idAdv);
//            $data['detail'] = $this->trans_masuk_m->cetak_cpa_detail($idAdv);
