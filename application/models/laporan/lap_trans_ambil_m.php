...
class Lap_trans_ambil_m extends CI_Model {
    public function getCucianAmbil($tglAwal, $tglAkhir,$status_Bayar) {
        if($status_Bayar == ''){
           $status_Bayar = ""; 
        }else if($status_Bayar == 0){
          $status_Bayar = " and status_bayar = 0";  
        }else if($status_Bayar == 1){
          $status_Bayar = " and status_bayar = 1";  
        }
        $sql = "SELECT mc.id_master_cm, mc.no_bon_manual,mc.id_cust,mc.tgl_trans,mc.e_tgl_selesai,
            ...
            where mc.tgl_ambil between '$tglAwal' and '$tglAkhir' and mc.status_ambil= 1 ".$status_Bayar." ";
