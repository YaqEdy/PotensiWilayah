...
class Trans_keluar_m extends CI_Model {
    ...
    public function getRencanaOutAll() {
        $sql = "SELECT * from master_out mo left join master_customer mc on mo.id_cust = mc.id_cust where status_cukai = '0'";
    ...
    function insertTransOutCukai($data) {
        $this->db->trans_begin();
        $model = $this->db->insert('trans_out_cukai', $data);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return false;
        } else {
