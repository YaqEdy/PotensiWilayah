...
class Master_su_m extends CI_Model {
    

    public function getSuAll() {
        $sql = "SELECT * from master_su kt "
                . "left join master_kecamatan kc on kt.id_kec = kc.id_kec "
                . "left join master_kelurahan kl on kt.id_kel = kl.id_kel "
                . "left join master_jenissentra js on kt.id_jenissentra = js.id_jenissentra ";
        $query = $this->db->query($sql);
        return $query->result(); // returning rows, not row
    }

    public function getIdSu() {
        $sql = "select id_su from master_su";
        $query = $this->db->query($sql);
        $jml = $query->num_rows();
        if ($jml == 0) {
            $id_kel = "000001";
            ...
            $sql = "select max(right(id_su,6)) as id_kel from master_su";
            $query = $this->db->query($sql);
            $hasil = $query->result();
    ...
    public function insert($data) {
        ...
        $this->db->trans_begin();
        $model = $this->db->insert('master_su', $data);
    ...
    function update($data, $kelId) {
        ...
        $query2 = $this->db->update('master_su', $data);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return false;
        } else {
    ...
    function delete($kelId) {
        $this->db->trans_begin();
        $query1 = $this->db->where('id_kel', $kelId);
        $query2 = $this->db->delete('master_su');
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return false;
        } else {
