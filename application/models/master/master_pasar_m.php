...
class Master_pasar_m extends CI_Model {
    ...
    public function getPasarAll() {
        $sql = "SELECT * from master_pasar kt "
    ...
    public function getIdPasar() {
        $sql = "select id_pasar from master_pasar";
        $query = $this->db->query($sql);
        $jml = $query->num_rows();
        if ($jml == 0) {
            $id_kel = "000001";
            ...
            $sql = "select max(right(id_pasar,6)) as id_kel from master_pasar";
            $query = $this->db->query($sql);
            $hasil = $query->result();
    ...
    public function insert($data) {
        ...
        $this->db->trans_begin();
        $model = $this->db->insert('master_pasar', $data);
    ...
    function update($data, $kelId) {
        ...
        $query2 = $this->db->update('master_pasar', $data);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return false;
        } else {
    ...
    function delete($kelId) {
        $this->db->trans_begin();
        $query1 = $this->db->where('id_kel', $kelId);
        $query2 = $this->db->delete('master_pasar');
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return false;
        } else {
