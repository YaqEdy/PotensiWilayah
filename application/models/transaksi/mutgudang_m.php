...
class Mutgudang_m extends CI_Model {
    ...
    function getIdMasterMutasi($bulan, $tahun) {
        $sql = "select id_master_mutasi from master_mutasi where MONTH(tgl_trans)='$bulan' and YEAR(tgl_trans)='$tahun'";
        $query = $this->db->query($sql);
        $jml = $query->num_rows();
        $kode = "MT";
        $th = substr($tahun, -2);
        if ($jml == 0) {
            ...
            $sql = "select max(substring(id_master_mutasi,4,7)) as id_adv from master_mutasi where MONTH(tgl_trans)='$bulan' and YEAR(tgl_trans)='$tahun'";
            $query = $this->db->query($sql);
            $hasil = $query->result();
    ...
    function insertMaster($data) {
        $this->db->trans_begin();
        $model = $this->db->insert('master_mutasi', $data);
