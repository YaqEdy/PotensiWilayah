        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-red-sunglo bold uppercase">Data Pekerjaan</span>
                </div>
                <div class="tools">
...
<!--                                                         <th>
                      ...
                      action="<?php echo base_url('master/master_pekerjaan/home'); ?>" id="id_formPekerjaan">
                                ...
                                <div class="form-body">
                                    <div class="col-md-6">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="input-group">
                                                        <input id="id_PekerjaanId" required="required" class="form-control input-sm"
                                                               type="text" name="PekerjaanId" readonly/>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <input id="id_namaPekerjaan" required="required" class="form-control input-sm"
                                                   type="text" name="namaPekerjaan"/>
                                        </div>
<!--                                        <div class="form-group">
                                            <label>Alamat</label>
...
<script>
    ...
    var TableManaged = function () {
        ...
        var initTable1 = function () {
            ...
            var table = $('#idTabelPekerjaan');

            // begin first table
            table.dataTable({
                "ajax": "<?php echo base_url("/master/master_pekerjaan/getPekerjaanAll"); ?>",
                "columns": [
                    {"data": "idPekerjaan"},
                    {"data": "namaPekerjaan"}
                    /*,
            ...
            table.on('click', 'tbody tr', function () {
                var idPekerjaan = $(this).find("td").eq(0).html();

                $('#id_PekerjaanId').val(idPekerjaan);
                getDescPekerjaan(idPekerjaan);
    ...
    $("#id_PekerjaanId").focusout(function () {
        var idPekerjaan = $(this).val();
        getDescPekerjaan(idPekerjaan);
    });
    function getDescPekerjaan(idPekerjaan) {
        ajaxModal();
        if (idPekerjaan != '') {
            $.post("<?php echo site_url('/master/master_pekerjaan/getDescPekerjaan'); ?>",
                    {
                        'idPekerjaan': idPekerjaan
                    }, function (data) {
                if (data.baris == 1) {
                    $('#id_namaPekerjaan').val(data.nama_pekerjaan);
    ...
    function ajaxSubmit() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo base_url(); ?>master/master_pekerjaan/simpan",
            data: dataString,
    ...
    function ajaxUbah() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo base_url(); ?>master/master_pekerjaan/ubah",
            data: dataString,
            success: function (data) {
                $('#id_Reload').trigger('click');
                $('#id_btnBatal').trigger('click');
    ...
    function ajaxHapus() {
        ajaxModal();
        var idPekerjaan = $('#id_PekerjaanId').val();
        idPekerjaan = idPekerjaan.trim();
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo base_url(); ?>master/master_pekerjaan/hapus",
    ...
    $('#id_formPekerjaan').submit(function (event) {
        dataString = $("#id_formPekerjaan").serialize();
