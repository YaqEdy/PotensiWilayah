<style type="text/css">
    table#idTabelOutsource td:nth-child(3) {
        text-align: right;
    }
    table#idTabelOutsource td:nth-child(4) {
        text-align: right;
    }
    table#idTabelOutsource td:nth-child(5) {
        text-align: right;
    }
    table#idTabelOutsource td:nth-child(6) {
        text-align: right;
    }
</style>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade" id="tab_2_2">
                        <form role="form" method="post" class=""
                              action="<?php echo base_url('master/master_outsource/home'); ?>" id="id_formOutsource">
                            <div class="row">
                                <div class="form-body">
                                    <div class="col-md-6">
                                        <div class="form-group hidden">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <label>Id Outsource </label>
                                                    <div class="input-group">
                                                        <input id="id_outsourceId" required="required" class="form-control input-sm"
                                                               type="text" name="outsourceId" readonly/>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label>Nama outsource</label>
                                            <input id="id_namaOutsource" required="required" class="form-control input-sm"
                                                   type="text" name="namaOutsource"/>
                                        </div>
                                        <div class="form-group">
                                            <label>Alamat</label>
...
<script>
    ...
    var TableManaged = function () {
        ...
        var TabelOutsource = function () {

            var table = $('#idTabelOutsource');

            // begin first table
            table.dataTable({
                "ajax": "<?php echo base_url("/master/master_outsource/getOutsourceAll"); ?>",
                "columns": [
                    {"data": "idOutsource"},
                    {"data": "namaOutsource"},
                    {"data": "alamat"},
            ...
            table.on('click', 'tbody tr', function () {
                var idOutsource = $(this).find("td").eq(0).html();
                $('#id_outsourceId').val(idOutsource.trim());
                var namaOutsource = $(this).find("td").eq(1).html();
                $('#id_namaOutsource').val(namaOutsource.trim());
    ...
    $("#id_outsourceId").focusout(function () {
        var idOutsource = $(this).val();
    });
    function ajaxSubmit() {
        ajaxModal();
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo base_url(); ?>master/master_outsource/simpan",
            data: dataString,
    ...
    function ajaxUbah() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo base_url(); ?>master/master_outsource/ubah",
            data: dataString,
            success: function (data) {
                $('#id_Reload').trigger('click');
                $('#id_btnBatal').trigger('click');
    ...
    function ajaxHapus() {
        ajaxModal();
        var idOutsource = $('#id_outsourceId').val();

        idOutsource = idOutsource.trim();
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo base_url(); ?>master/master_outsource/nonaktif",
            data: {idOutsource: idOutsource},
            success: function (data) {
                $('#id_Reload').trigger('click');
                $('#id_btnBatal').trigger('click');
                UIToastr.init(data.tipePesan, data.pesan);
    ...
    $('#id_formOutsource').submit(function (event) {
        dataString = $("#id_formOutsource").serialize();
