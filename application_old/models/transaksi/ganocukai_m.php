...
class Ganocukai_m extends CI_Model {
    ...
    public function getNoCukaiExist($noSO) {
        $sql = "select no_cukai from master_out where id_master_out = '$noSO' ";
        $query = $this->db->query($sql);
        if ($query->num_rows() <> 0) {
            $result =  $query->result();
            $noCukai = $result[0]->no_cukai;
            return $noCukai;
        } else {
            return '';
        }
